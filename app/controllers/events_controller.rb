class EventsController < ApplicationController
  ...
  def show
    ...
    if @event.approved == false
      if user_signed_in?
        if @event.host.id==current_user.id || current_user.id==1 || current_user.id==13 || current_user.id==525 || current_user.id==526 || current_user.id==115 || current_user.id==420 || current_user.id==17 || current_user.id==259 || current_user.id==184
          respond_to do |format|
            format.html
